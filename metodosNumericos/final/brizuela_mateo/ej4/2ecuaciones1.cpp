...
void rk3  (int modo, int N, double h, double a, double b, double x0, double y10, double y20);
...
// RK3 acoplado ACA ESTA EL CODIGO MODIFICADO CON LO QUE SE PIDIO EN EL EJERCICIO
void rk3(int modo, int N, double h, double a, double b, double x0, double y10, double y20) {
    const double eps = 1e-12;
    int P;
    calcularMallado(modo, N, h, a, b, &N, &h, &P);

    if (fabs(x0 - a) > eps) { printf("[RK3] x0!=a -> ajustado\n"); x0 = a; }
    ...
    for (int i=1; i<P; ++i) {
        ...
        // k1
        double k1_1 = h * f1(xn, y1n, y2n);
        double k1_2 = h * f2(xn, y1n, y2n);
        // k2
        double k2_1 = h * f1(xn + h/2.0, y1n + (1.0/2.0)*k1_1, y2n + (1.0/2.0)*k1_2);
        double k2_2 = h * f2(xn + h/2.0, y1n + (1.0/2.0)*k1_1, y2n + (1.0/2.0)*k1_2);
        // k3
        double k3_1 = h * f1(xn + h, y1n - k2_1 + 2.0*k2_1 , y2n - k2_2 + 2.0*k2_2);
        double k3_2 = h * f2(xn + h, y1n - k2_1 + 2.0*k2_1 , y2n - k2_2 + 2.0*k2_2);
        y1[i] = y1n + (1.0/6.0) * (k1_1 + 4.0*k2_1 + k3_1);
        y2[i] = y2n + (1.0/6.0) * (k1_2 + 4.0*k2_2 + k3_2);
    }
    if (modo==1) x[P-1]=b;

    printf("[RK3] modo=%d N=%d P=%d h=%.6f\n", modo, N, P, h);
    printf("i\t x\t\t y1\t\t y2\n");
    for (int i=0;i<P;++i)
        printf("%d\t%.8f\t%.10f\t%.10f\n", i, x[i], y1[i], y2[i]);

    exportarDerivadas("rk3_y1", x, y1, P);
    exportarDerivadas("rk3_y2", x, y2, P);
