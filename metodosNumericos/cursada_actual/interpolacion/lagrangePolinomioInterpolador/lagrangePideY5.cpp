/**
 * @file lagrangePideY5.cpp
 * @brief Implementación del método de interpolación polinómica de Lagrange
 * 
 * @description
 * Este programa calcula el polinomio interpolante de Lagrange para un conjunto
 * de puntos dados. Permite al usuario:
 * - Ingresar puntos manualmente o generarlos mediante una función matemática
 * - Evaluar el polinomio interpolante en cualquier punto
 * - Visualizar el polinomio en diferentes formatos
 * 
 * CAMBIOS RESPECTO A LA VERSIÓN ANTERIOR:
 * - Se agregó la función imprimirPolinomioSimplificado() que calcula y muestra
 *   el polinomio interpolante en su forma canónica expandida:
 *   P(x) = c₀ + c₁x + c₂x² + ... + cₙ₋₁xⁿ⁻¹
 * - Esta representación simplificada facilita copiar el polinomio directamente
 *   a herramientas de graficación como GeoGebra o WolframAlpha
 * - Se muestran los coeficientes individuales con alta precisión (12 decimales)
 * 
 * @note El grado del polinomio interpolante es n-1, donde n es el número de puntos
 * @warning Todos los valores x_i deben ser distintos para evitar división por cero
 * 
 * @author Mateo Brizuela
 * @date 2024
 */
//
