...
int main(int argc, char const *argv[])
{
    ...
    tablaIncisoB(x, incognitas);           // inciso b: tabla equiespaciada (conservar d)
...
void mostrarCoeficientes(const double x_nodes[d], const double coeficientes[n]) {
    ...
    for (int k = 0; k < m; ++k) {
        ...
        printf("S%d [%.6f, %.6f]: ", k, x_nodes[k], x_nodes[k+1]);
        printf("a=%.8e  b=%.8e  c=%.8e  d=%.8e\n",
