...
void heun() {
    ...
    for (int i = 1; i < P; ++i) {
        x[i] = x[i - 1] + h;
        // predictor (Euler explícito) al punto x[i] = x_{n+1}
        double y_tilde = y[i - 1] + h * f(x[i - 1], y[i - 1]);
        // corrector (Heun / trapecio explícito)
        y[i] = y[i - 1] + (h / 2.0) * ( f(x[i - 1], y[i - 1]) + f(x[i], y_tilde) );
    }
