...
void puntoMedio() {
    // Entradas
    double a = 0.0, b = 0.0;
    double x0 = 0.0, y0 = 1.0, xp = 0.0, yp = 0.0;
    int modo = 1; // 1: definir N; 2: definir h
    const double eps = 1e-12;

    printf("Metodo de punto medio para EDO y' = f(x,y)\n");
    ...
    // Mallado y punto medio explícito
    x[0] = x0;   // = a
    y[0] = y0;
    for (int i = 1; i < P; ++i) {
        x[i] = x[i - 1] + h;
    ...
    // Resultados
    printf("\nResultados de la aproximacion por el metodo de punto medio:\n");
