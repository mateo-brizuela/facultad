// programa con rk4 alternativo rk4(3/8)
#include <stdio.h>
#include <cmath>
...
void rk4() {
    ...
    double k1, k2, k3, k4;
    // modificacion del bucle para rk4(3/8) ACA ESTA LA DIFERENCIA
    for (int i = 1; i < P; ++i) {
        
        k1 =f(x[i - 1], y[i - 1]);
        k2 =f(x[i - 1] + h / 3.0, y[i - 1] + (k1 / 3.0)*h);
        k3 =f(x[i - 1] + 2.0 * h / 3.0, y[i - 1] + (-k1 / 3.0 + k2)*h);
        k4 =f(x[i - 1] + h, y[i - 1] + (k1 - k2 + k3)*h);

        x[i] = x[i - 1] + h;
        y[i] = y[i - 1] + (h / 8.0) * (k1 + 3.0 * k2 + 3.0 * k3 + k4);
    }
