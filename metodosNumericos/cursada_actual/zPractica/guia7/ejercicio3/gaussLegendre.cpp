...
double func(double x){ // funcion matematica
    //return (x*x) + 1.0; // funcion de ejemplo
    return 2.0 + sin(2.0 * pow(x, 0.5)); // funcion de ejemplo
}
...
void gauss_legendre(){
    ...
    switch (n) {
        case 2: {
            double xm = a + 0.5 * (b - a);
            double xr = 0.5 * (b - a);
            double x0 = xm + xr * (-0.577350269);
            double x1 = xm + xr * ( 0.577350269);
            double c0 = 1.0000000, c1 = 1.0000000;
            integral = xr * (c0 * func(x0) + c1 * func(x1));
            break;
        }
