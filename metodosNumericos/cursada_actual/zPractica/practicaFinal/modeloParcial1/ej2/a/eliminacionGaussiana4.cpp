...
void armar (double A[n][n], double B[n]);
...
void armar(double A[n][n], double B[n]) {
    ...
    int i, j; //recorre las filas de la matriz, j recorre columnas  
    ...
    // cargo en la primera y ultima fila 
    A[0][0] = 2.0;
    A[0][1] = 1.0;
    A[n-1][n-2] = 1.0;
    A[n-1][n-1] = 2.0;

    // cargo el interior de la matriz
    for ( i = 1; i < n-1; i++)
    {
        A[i][i-1] = 1.0;
        A[i][i] = 2.0;
...
void gauss(double A[n][n], double B[n], double x[n]) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (fabs(A[i][i]) < ceroDef) { 
            ...
            if (p != i) { // Realizar intercambio de filas
                intercambios++;
                double aux;
                for (int m = 0; m < n; m++) {
                    aux = A[p][m];
