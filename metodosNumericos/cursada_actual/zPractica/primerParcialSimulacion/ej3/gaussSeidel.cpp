...
void cargar(double matriz[n][n], double vector[n]){
    ...
    // armando la matriz como plantea el ejercicio

  // Primera ecuación: x₁ = 1
    matriz[0][0] = 1.0;
    vector[0] = 1.0;
    
    // Ecuaciones del medio: x_{i-1} - 2x_i + x_{i+1} = 4
    for (int i = 1; i < n-1; i++) {
        matriz[i][i-1] = 1.0;   // coeficiente de x_{i-1}
        matriz[i][i]   = -2.0;  // coeficiente de x_i
        matriz[i][i+1] = 1.0;   // coeficiente de x_{i+1}
        vector[i] = 4.0;
    }
    
    // Última ecuación: x_{100} = 1
    matriz[n-1][n-1] = 1.0;
    vector[n-1] = 1.0;
    
    printf("Matriz y vector cargados correctamente\n");
    printf("\n");
}
