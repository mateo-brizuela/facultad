...
int main() {
    ...
    if (manual) {
        ...
        printf("ingrese el valor de epsilon(el numero lo suficientemente chico para asumir 0): ");
        if (scanf("%lf",&epsilon) != 1) {printf("entrada invalida.\n"); return 1;}
    } else {
        // Defaults de ejemplo (ajustalos a tu problema)
        a = 0.0;
        b = 1.0;
        epsilon = 0.00001;
        printf("Usando defaults: a=%.6f, b=%.6f, epsilon = %.10f\n", a, b, epsilon);
    }
    ...
    do {
        ...
        if (fc < epsilon) {
            ...
            printf("Raiz(aproximadamente)= %.10f\n", c);
            printf("Iteraciones: %d\n", i);
            return 0;
        }
