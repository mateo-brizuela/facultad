...
int main() {
    // Variables
    double a = 0.0, b = 0.0, c = 0.0, cViejo = 3.2; // MODIFICO EL CVIEJO PORQUE LO PIDE EL EJERCICIO XVIEJO
    ...
    do {
        ...
        if (primera) {
            ...
            eAbs = fabs(c - cViejo);
            if (c != 0.0) eRel = ( (fabs(c-1.75)) / (0.5 * fabs( c -1.75)) ) * 100.0; //ESTO LO MODIFICAMOS POQUE SE PIDE EN EL EJERCICIO
